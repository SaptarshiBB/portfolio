...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 h-20 z-50 transition-all duration-500 ${scrolled ? 'backdrop-blur-xl bg-slate-950/60 border-b border-white/5 shadow-2xl' : 'bg-transparent'}`}>
        ...
        <div className="flex items-center justify-between h-full">
          ...
          <div className="flex-1 flex justify-start">
            <NavLink to="/" className="flex items-center gap-3 group">
              ...
              <span className="text-white font-black text-xl tracking-tighter hover:text-primary transition-colors">Saptarshi<span className="text-primary">.dev</span></span>
            </NavLink>
          </div>
          ...
          <div className="flex-1 flex justify-end items-center gap-5">
            <button
              onClick={toggleTheme}
              className="w-11 h-11 rounded-2xl flex items-center justify-center bg-white/5 border border-white/10 text-slate-300 hover:bg-white/10 hover:text-white transition-all duration-300 hover:shadow-neon"
      ...
      <div className={`lg:hidden fixed inset-x-0 top-20 transition-all duration-500 overflow-hidden ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="container-max pb-8">
          <div className="card-glass border-t-0 rounded-t-none bg-slate-950/95 flex flex-col gap-4">
            {navLinks.map(({ to, label }) => (
              <NavLink
                key={to} to={to} end={to === '/'}
                className={({ isActive }) =>
